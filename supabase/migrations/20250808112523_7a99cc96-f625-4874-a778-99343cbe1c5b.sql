-- Phase 1: Teacher Portal schema, RLS, and storage policies (retry without IF NOT EXISTS)
...
-- teaching_resources
DO $$ BEGIN
  CREATE POLICY "Admins manage all teaching resources"
...
DO $$ BEGIN
  CREATE POLICY "Teachers view global approved resources"
  ON public.teaching_resources FOR SELECT
  USING (
...
-- resource_shares
DO $$ BEGIN
  CREATE POLICY "Admins manage all resource_shares"
...
DO $$ BEGIN
  CREATE POLICY "Owners manage shares of their resources"
  ON public.resource_shares FOR ALL
  USING (
    EXISTS (
...
-- progress_notes
DO $$ BEGIN
  CREATE POLICY "Admins manage all progress notes"
...
DO $$ BEGIN
  CREATE POLICY "Teachers manage own progress notes"
  ON public.progress_notes FOR ALL
...
-- 8) Storage policies for teacher-documents
DO $$ BEGIN
...
DO $$ BEGIN
  CREATE POLICY "Teachers can read shared resources"
  ON storage.objects FOR SELECT TO authenticated
  USING (
    bucket_id = 'teacher-documents'
    AND EXISTS (
...
DO $$ BEGIN
  CREATE POLICY "Teachers can read global approved resources"
  ON storage.objects FOR SELECT TO authenticated
  USING (
    bucket_id = 'teacher-documents'
    AND EXISTS (
...
DO $$ BEGIN
  CREATE POLICY "Admins can access teacher-documents"
